...
//USART0 
    namespace Nonecfg{    ///<USART Configuration register. Basic USART configuration settings that typically are not changed during operation.
        ...
        enum class mode32kVal {
            ...
            uartUsesThe32Khz=0x00000001,     ///<UART uses the 32 kHz clock from the RTC oscillator as the clock source to the BRG, and uses a special bit clocking scheme.
        };
        namespace mode32kValC{
            constexpr MPL::Value<mode32kVal,mode32kVal::uartUsesStandardC> uartUsesStandardC{};
            constexpr MPL::Value<mode32kVal,mode32kVal::uartUsesThe32Khz> uartUsesThe32Khz{};
        }
    ...
    namespace Nonectl{    ///<USART Control register. USART control settings that are more likely to change during operation.
        ...
        namespace clrcconrxValC{
            constexpr MPL::Value<clrcconrxVal,clrcconrxVal::noAffectOnTheCc> noAffectOnTheCc{};
        ...
        enum class autobaudVal {
            ...
            enabled=0x00000001,     ///<Enabled. UART is in autobaud mode. This bit should only be set when the UART receiver is idle. The first start bit of RX is measured and used the update the BRG register to match the received data rate. AUTOBAUD is cleared once this process is complete, or if there is an AERR.
        };

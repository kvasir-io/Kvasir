...
//Pulse Width Modulation Unit 1
    namespace Pwm1TasksStop{    ///<Stops PWM pulse generation on all channels at the end of current PWM period, and stops sequence playback
        using Addr = Register::Address<0x40021004,0xffffffff,0x00000000,unsigned>;
    }
    namespace Pwm1TasksNextstep{    ///<Steps by one value in the current sequence on all enabled channels if DECODER.MODE=NextStep. Does not cause PWM generation to start it was not running.
        using Addr = Register::Address<0x40021010,0xffffffff,0x00000000,unsigned>;
    }
    namespace Pwm1EventsStopped{    ///<Response to STOP task, emitted when PWM pulses are no longer generated
        using Addr = Register::Address<0x40021104,0xffffffff,0x00000000,unsigned>;
    }
    namespace Pwm1EventsPwmperiodend{    ///<Emitted at the end of each PWM period
        using Addr = Register::Address<0x40021118,0xffffffff,0x00000000,unsigned>;
    }
    namespace Pwm1EventsLoopsdone{    ///<Concatenated sequences have been played the amount of times defined in LOOP.CNT
        using Addr = Register::Address<0x4002111c,0xffffffff,0x00000000,unsigned>;
    }
    namespace Pwm1Shorts{    ///<Shortcut register
        using Addr = Register::Address<0x40021200,0xffffffe0,0x00000000,unsigned>;
    ...
    namespace Pwm1Inten{    ///<Enable or disable interrupt
        using Addr = Register::Address<0x40021300,0xffffff01,0x00000000,unsigned>;
    ...
    namespace Pwm1Intenset{    ///<Enable interrupt
        using Addr = Register::Address<0x40021304,0xffffff01,0x00000000,unsigned>;
    ...
    namespace Pwm1Intenclr{    ///<Disable interrupt
        using Addr = Register::Address<0x40021308,0xffffff01,0x00000000,unsigned>;
    ...
    namespace Pwm1Enable{    ///<PWM module enable register
        using Addr = Register::Address<0x40021500,0xfffffffe,0x00000000,unsigned>;
    ...
    namespace Pwm1Mode{    ///<Selects operating mode of the wave counter
        using Addr = Register::Address<0x40021504,0xfffffffe,0x00000000,unsigned>;
    ...
    namespace Pwm1Countertop{    ///<Value up to which the pulse generator counter counts
        using Addr = Register::Address<0x40021508,0xffff8000,0x00000000,unsigned>;
    ...
    namespace Pwm1Prescaler{    ///<Configuration for PWM_CLK
        using Addr = Register::Address<0x4002150c,0xfffffff8,0x00000000,unsigned>;
    ...
    namespace Pwm1Decoder{    ///<Configuration of the decoder
        using Addr = Register::Address<0x40021510,0xfffffef8,0x00000000,unsigned>;
    ...
    namespace Pwm1Loop{    ///<Amount of playback of a loop
        using Addr = Register::Address<0x40021514,0xffff0000,0x00000000,unsigned>;
        ///Amount of playback of pattern cycles
        constexpr Register::FieldLocation<Addr,Register::maskFromRange(15,0),Register::ReadWriteAccess,unsigned> cnt{}; 
    }
    namespace Pwm1TasksSeqstart0{    ///<Description collection[0]:  Loads the first PWM value on all enabled channels from sequence 0, and starts playing that sequence at the rate defined in SEQ[0]REFRESH and/or DECODER.MODE. Causes PWM generation to start it was not running.
        using Addr = Register::Address<0x40021008,0xffffffff,0x00000000,unsigned>;
    }
    namespace Pwm1TasksSeqstart1{    ///<Description collection[0]:  Loads the first PWM value on all enabled channels from sequence 0, and starts playing that sequence at the rate defined in SEQ[0]REFRESH and/or DECODER.MODE. Causes PWM generation to start it was not running.
        using Addr = Register::Address<0x4002100c,0xffffffff,0x00000000,unsigned>;
    }
    namespace Pwm1EventsSeqstarted0{    ///<Description collection[0]:  First PWM period started on sequence 0
        using Addr = Register::Address<0x40021108,0xffffffff,0x00000000,unsigned>;
    }
    namespace Pwm1EventsSeqstarted1{    ///<Description collection[0]:  First PWM period started on sequence 0
        using Addr = Register::Address<0x4002110c,0xffffffff,0x00000000,unsigned>;
    }
    namespace Pwm1EventsSeqend0{    ///<Description collection[0]:  Emitted at end of every sequence 0, when last value from RAM has been applied to wave counter
        using Addr = Register::Address<0x40021110,0xffffffff,0x00000000,unsigned>;
    }
    namespace Pwm1EventsSeqend1{    ///<Description collection[0]:  Emitted at end of every sequence 0, when last value from RAM has been applied to wave counter
        using Addr = Register::Address<0x40021114,0xffffffff,0x00000000,unsigned>;
    }
    namespace Pwm1TasksSeqstart0{    ///<Description collection[0]:  Loads the first PWM value on all enabled channels from sequence 0, and starts playing that sequence at the rate defined in SEQ[0]REFRESH and/or DECODER.MODE. Causes PWM generation to start it was not running.
        using Addr = Register::Address<0x40021008,0xffffffff,0x00000000,unsigned>;
    }

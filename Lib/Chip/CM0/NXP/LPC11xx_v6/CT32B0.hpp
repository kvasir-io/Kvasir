...
//Product name title=UM10398 Chapter title=LPC1100XL series: 32-bit counter/timer CT32B0/1 Modification date=2/22/2012 Major revision=8 Minor revision=not available 
    namespace Noneir{    ///<Interrupt Register (IR). The IR can be written to clear interrupts. The IR can be read to identify which of five possible interrupt sources are pending.
        using Addr = Register::Address<0x40014000,0xffffffc0,0,unsigned>;
...
interrupt every time MR matches the TC.
    ...
    namespace Nonemr1{    ///<Match Register. MR can be enabled through the
MCR to reset the TC, stop both the TC and PC, and/or generate an
interrupt every time MR matches the TC.
    ...
    namespace Nonemr2{    ///<Match Register. MR can be enabled through the
MCR to reset the TC, stop both the TC and PC, and/or generate an
interrupt every time MR matches the TC.
    ...
    namespace Nonemr3{    ///<Match Register. MR can be enabled through the
MCR to reset the TC, stop both the TC and PC, and/or generate an
interrupt every time MR matches the TC.
    ...
    namespace Nonecr1{    ///<Capture Register (CR). CR is loaded with the value of TC when there is an event on the CT16Bn_CAPm input.
    ...
    namespace Nonectcr{    ///<Count Control Register (CTCR). The CTCR selects between Timer and Counter mode, and in Counter mode selects the signal and edge(s) for counting.
        ...
        namespace CisValC{
            ...
            constexpr Register::FieldValue<decltype(cis),CisVal::ct32bnCap1> ct32bnCap1{};
            constexpr Register::FieldValue<decltype(cis),CisVal::reservedNoteIfCo> reservedNoteIfCo{};
        }
        ...
        namespace SelccValC{
            constexpr Register::FieldValue<decltype(selcc),SelccVal::risingEdgeOfCap0> risingEdgeOfCap0{};
            constexpr Register::FieldValue<decltype(selcc),SelccVal::fallingEdgeOfCap0> fallingEdgeOfCap0{};
            constexpr Register::FieldValue<decltype(selcc),SelccVal::risingEdgeOfCap1> risingEdgeOfCap1{};
            constexpr Register::FieldValue<decltype(selcc),SelccVal::fallingEdgeOfCap1> fallingEdgeOfCap1{};
        }
    }
    namespace Nonepwmc{    ///<PWM Control Register (PWMCON). The PWMCON enables PWM mode for the external match pins CT32B0_MAT[3:0].
        ...
        namespace Pwmen1ValC{
            constexpr Register::FieldValue<decltype(pwmen1),Pwmen1Val::ct32bnMat1IsContr> ct32bnMat1IsContr{};

...
namespace Kvasir
{
	namespace Usb
	{
		...
		public:
			...
			uint8_t* unsafeToBufPointer()
			{
				return packet_->buf_;
			}
			static Packet unsafeFromPacketPointer(PointerType p)
			{
				return Packet(p);
			}
			PointerType unsafeToPacketPointer()
			{
				return packet_;
			}
			void unsafeSetSize(int i)
			{
				packet_->size_ = i;
			}
			uint8_t getSize()
			{
				return packet_->size_;
			}
			operator bool() const
			{
			...
			void setNext(PointerType &p)
			{
				packet_->next_ = p;
			}
			bool isData1()
			{
				return packet_->endpoint_ & 0x80;
			}
			void makeData1()
			{
				packet_->endpoint_ |= 0x80;
			}
			void makeData0()
			{
				packet_->endpoint_ &= ~0x80;
			...
			{
				packet_->endpoint_ |= (ep.value_ & 0x1F);
			} 
			void pushBack(uint8_t data)
			{
				packet_->pushBack(data);
			}
		};
	}
}
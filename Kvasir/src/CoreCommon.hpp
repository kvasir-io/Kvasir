...
namespace Kvasir{
	namespace Core{
		...
		struct EnableIrq;
		template<int I>
		struct EnableIrq<Interrupt::Type<I>> : Register::OptionT<Detail::getIrqRegister(Detail::IsrRegister::ISER,I),0x0,(1<<I)>{};
		template<typename T>
		...
		struct DisableIrq;
		template<int I>
		struct DisableIrq<Interrupt::Type<I>> : Register::OptionT<Detail::getIrqRegister(Detail::IsrRegister::ICER,I),0x0,(1<<(I & 0x1F))>{};
		template<typename T>
		...
		template<int I>
		struct SetPendingIrq<Interrupt::Type<I>> : Register::OptionT<Detail::getIrqRegister(Detail::IsrRegister::ISPR,I),0x0,(1<<(I & 0x1F))>{};
		template<typename T>
		...
		template<int I>
		struct ClearPendingIrq<Interrupt::Type<I>> : Register::OptionT<Detail::getIrqRegister(Detail::IsrRegister::ICPR,I),0x0,(1<<(I & 0x1F))>{};
		template<typename T>
